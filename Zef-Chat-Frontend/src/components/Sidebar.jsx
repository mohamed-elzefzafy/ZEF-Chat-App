...
useEffect(()=>{
  ...
  return (
    ...
    <header className="p-4 border-b border-gray-300 flex justify-between items-center bg-indigo-600 text-white">
      <h1 className="text-1xl font-semibold">Chat Web</h1>
      {/* <div className='flex-1'> */}
      <button
       onClick={generatenewchatId}
       className='float-right  border-2 border-blue-800 font-semibold rounded-md min-w-24 justify-self-end hover:scale-[1.05] transition-all duration-200'>
      New chat</button>
    {/* </div> */}
    ...
    {chats?.map((chat) => 
      ...
      <div onClick={() =>{ window.location.href = `http://localhost:5173/?chatid=${chat?.name}`} } key={chat?.name} className="flex items-center mb-4 cursor-pointer hover:bg-gray-100 p-2 rounded-md">
        ...
        <div className="flex-1">
          <h2 className="text-lg font-semibold">{chat?.name || "General"}</h2>
